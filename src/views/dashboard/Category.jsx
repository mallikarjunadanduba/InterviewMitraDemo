...
// Using Material-UI icons for better compatibility

// Course icon mapping function using Material-UI icons
const getCourseIcon = (categoryName) => {
  ...
  // Default fallback
  return <School style={{ fontSize: '2rem' }} />;
};
  ...
  const handleCategoryClick = async (categoryId, categoryName) => {
    try {
      ...
      if (fetchedData.length > 0) {
        navigate("/upSkills/courses", { state: { categoryId, categoryName, details: fetchedData } });
      } else {
    ...
    <Box sx={{ position: 'relative' }}>
      ...
      <Grid container spacing={2}>
        {(showAll ? categories : categories.slice(0, 8)).map((category, index) => {
          const courseIcon = getCourseIcon(category.categoryName);
